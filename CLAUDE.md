...
- recharts 2 (心拍グラフ)
- Tailwind CSS 4
- `next.config.ts` の rewrites で `/api/*` を backend にプロキシ (HTTP)
- `custom-server.js` で `/api/ws/*` の WebSocket upgrade を backend にプロキシ
...
- [x] Frontend: custom-server.js (WS プロキシ)
- [x] Docker: Dockerfile (backend/frontend), docker-compose.yml, cloudflared, redis
- [ ] README.md 更新
